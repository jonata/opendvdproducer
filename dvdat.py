...
if sys.platform == 'win32':
    path_dvdat = getattr(sys, '_MEIPASS', os.getcwd())

else:
    path_dvdat = os.path.dirname(sys.argv[0])

#os.path.realpath(__file__)#os.path.abspath(os.path.dirname(sys.argv[0]))

path_graphics = os.path.join(path_dvdat, 'graphics')
path_home = os.environ.get('HOME', None)

path_tmp = os.path.join(tempfile.gettempdir(), 'dvdat-' + str(random.randint(1000,9999)))

if sys.platform == 'darwin':
    iso2ddp_bin = os.path.join(path_dvdat, 'iso2ddp_mac')
    imagemagick_convert_bin = os.path.join(path_dvdat, 'convert')
    interface_font_size = 12
    ffprobe_bin = os.path.join(path_dvdat, 'ffprobe')
    ffmpeg_bin = os.path.join(path_dvdat,  'ffmpeg')
    spumux_bin = os.path.join(path_dvdat, 'spumux')
    dvdauthor_bin = os.path.join(path_dvdat, 'dvdauthor')
    mkisofs_bin = os.path.join(path_dvdat, 'mkisofs')
    md5_bin = '/sbin/md5'
    split_bin = '/usr/bin/split'

elif sys.platform == 'win32':
    iso2ddp_bin = os.path.join(path_dvdat, 'iso2ddp.exe')
    imagemagick_convert_bin = os.path.join(path_dvdat, 'convert.exe')
    ffprobe_bin = os.path.join(path_dvdat, 'ffprobe.exe')
    ffmpeg_bin = os.path.join(path_dvdat, 'ffmpeg.exe')
    spumux_bin = os.path.join(path_dvdat, 'spumux.exe')
    dvdauthor_bin = os.path.join(path_dvdat, 'dvdauthor.exe')
    mkisofs_bin = os.path.join(path_dvdat, 'mkisofs.exe')
    md5_bin = os.path.join(path_dvdat, 'md5.exe')
    split_bin = os.path.join(path_dvdat, 'split.exe')
    ...
    md5_bin = 'md5sum'
    split_bin = 'split'
    iso2ddp_bin = os.path.join(path_dvdat, 'resources', 'iso2ddp')
        ...
        def generate_ddp(original_file, output_path):
            ...
            for layer in layers:
                ...
                ddpid = str(
                    ...
                    " " +                                                       #/* Media Number */
                    "                                                " +        #/* Master ID (user supplied string) */
                    " " +                                                       #/* BK, reserved */
                    "DV" +                                                      #/* TYPE, Type of Disc ("DV" = DVD) */
                    str(sides) +                                                #/* NSIDE,  # of sides on final disc */
                    str(actual_side) +                                          #/* SIDE */
                    str(len(layers)) +                                          #/* NLAYER */
                    str(actual_layer) +                                         #/* LAYER */
                    "I" +                                                       #/* DIR.  Direction of translation for second layer. "I" = inner to outer, "O" = outer to inner (opposite path). */
                    disc_size +                                                 #/* Disc size "B" = 12cm, "A" = 8cm */
                    "0" +                                                       #/* SSCRST. Security Scrambling Status "0"  = No encryption. */
                    "0" +                                                       #/* SSCRMD. Security Scrambling Mode. "0" = DVD. */
                    "  " +                                                      #/* SIZ. Size of user text */
                    "                             " +                           #/* TXT. User text  */

                    "VVVM" +                                                    #/* MPV Map Packet Valid */
                    "D2" +                                                      #/* DST Data Stream Type "D2" = control  "D0" = main DVD */
                    "        " +                                                #/* DSP Data Stream Pointer, empty for tapes */
                    "00000016" +                                                #/* DSL Data Stream Length */
                    "00193024" +                                                #/* DSS Data Stream Start */
                    "        " +                                                #/* SUB Subcode descriptor */
                    "DV" +                                                      #/* CDM DVD Disc mode. */
                    "1" +                                                       #/* SSM Source Storage Mode */
                    ...
                    "CONTROL.DAT      " +                                       #/* DSI Data stream identifier                   ("CONTROL.DAT" or "IMAGE.DAT") */
                    " " +                                                       #/* NEW Reserved */
                    "    " +                                                    #/* PRE1NXT Reserved */
                    "        " +                                                #/* PAUSEADD Reserved. */
                    "         " +                                               #/* OFS Starting offset */
                    ...
                    "VVVM" +                                                    #/* MPV Map Packet Valid */
                    "D0" +                                                      #/* DST Data Stream Type "D2" = control  "D0" = main DVD */
                    "        " +                                                #/* DSP Data Stream Pointer, empty for tapes */
                    str("%08d" % int(layer_size)) +                              #/* DSL Data Stream Length */
                    "00196608" +                                                #/* DSS Data Stream Start */
                    "        " +                                                #/* SUB Subcode descriptor */
                    "DV" +                                                      #/* CDM DVD Disc mode. */
                    ...
                    "            " +                                            #/* ISRC Reserved */
                    "017"                                                       #/* SIZ Size of data stream identifier */
                    "MAIN.DAT         " +                                       #/* DSI Data stream identifier                   ("CONTROL.DAT" or "IMAGE.DAT") */
                    " " +                                                       #/* NEW Reserved */
                    "    " +                                                    #/* PRE1NXT Reserved */
                    "        " +                                                #/* PAUSEADD Reserved. */
                    "         " +                                               #/* OFS Starting offset */
                ...
                control_dat += str(bytearray([int('00000000', 2)]))
                control_dat += str(bytearray([int('00000000', 2)]))
                control_dat += str(bytearray([int('00000001', 2)]))                 #/* Byte 0 - Disk Category and Version Number */ - 0000 to increase compatibility http://web.archive.org/web/20090227032500/http://cdfreaks.com/reviews/Increased-compatibility-DVD-bitsetting
                                                                                    #/* Bits b0 to b3 shall specify the Version Number - They shall be set to 0001, indicating this ECMA Standard. Bits b4 to b7 shall specify the Disk Category - These bits shall be set to 0000, indicating a read-only disk. Other settings are prohibited by this ECMA Standard. */

                control_dat += str(bytearray([int('00000010', 2)]))                 #/* Byte 1 - Disk size and maximum transfer rate
                                                                                    #/* Bits b0 to b3 shall specify the maximum transfer rate.
                                                                                    #/* if set to 0000, they specify a maximum transfer rate of 2,52 Mbits/s
                                                                                    #/* if set to 0001, they specify a maximum transfer rate of 5,04 Mbits/s
                                                                                    #/* if set to 0010, they specify a maximum transfer rate of 10,08 Mbits/s
                                                                                    #/* Bits b4 to b7 shall specify the disk size - They shall be set to 0000, indicating a 120 mm disk

                if len(layers) == 2:
                    if tp_mode == "OTP":                                            #/* Byte 2 - Disk structure
                        control_dat += str(bytearray([int('00110001', 2)]))         #/* Bits b0 to b3 shall specify the type of the recorded layer(s) - They shall be set to 0001, indicating a read-only layer(s)
                    else:                                                           #/* Bit b4 shall specify the track path
                        control_dat += str(bytearray([int('00100001', 2)]))         #/* if set to ZERO, it specifies PTP on DL disks or a SL disk if set to ONE, it specifies OTP on DL disks
                else:                                                               #/* Bits b5 and b6 shall specify the disk Type
                    if tp_mode == "OTP":                                            #/* if set to 00, they specify Type A or Type B if set to 01, they specify Type C or Type D
                        control_dat += str(bytearray([int('00010001', 2)]))         #/* Bit b7 shall be set to ZERO.
                    else:
                        control_dat += str(bytearray([int('00000001', 2)]))

                control_dat += str(bytearray([int('00010000', 2)]))                 #/* Byte 3 - Recording density
                                                                                    #/* Bits b0 to b3 shall specify the average track pitch, they shall be set to 0000, indicating an average track pitch of 0,74 μm
                                                                                    #/* Bits b4 to b7 shall specify the average Channel bit length if set to 0000, they specify 0,133 μm
                                                                                    #/* if set to 0001, they specify 0,147 μm


                control_dat += str(bytearray([int('00000000', 2)]))                 #/* Bytes 4 to 15 - Data Zone allocation
                control_dat += str(bytearray([int('00000011', 2)]))                 #/* Byte 4 shall be set to (00).
                control_dat += str(bytearray([int('00000000', 2)]))                 #/* Bytes 5 to 7 shall be set to (030000) to specify the Sector Number 196 608 of the first Physical Sector of the Data Zone
                control_dat += str(bytearray([int('00000000', 2)]))                 #/* Byte 8 shall be set to (00).
                control_dat += str(bytearray([int('00000000', 2)]))                 #/* Bytes 9 to 11 shall specify the Sector Number of the last Physical Sector of the Data Zone
                control_dat += str(('%%0%dx' % (3 << 1) % int(196608 + main_size - 1)).decode('hex')[-3:])
                control_dat += str(bytearray([int('00000000', 2)]))                 #/* Byte 12 shall be set to (00)

                if tp_mode == "OTP" and len(layers) == 2:                           #/* Byte 13 to 15 shall be set to (00) on SL disks and DL disks in PTP mode, and to the Sector Number of the last Physical Sector of Layer 0 on DL disks in OTP mode.
                ...
                control_dat += str(bytearray([int('00000000', 2)]))                 #/* Byte 16 - BCA descriptor
                                                                                    #/* This byte shall specify whether or not there is a Burst Cutting Area on the disk. bits b0 to b6 shall be set to ZERO
                                                                                    #/* bit b7, the BCA flag, shall specify whether or not a BCA exists
                                                                                    #/* if set to ZERO, it shall indicate that a BCA does not exist
                                                                                    #/* if set to ONE, it shall indicate that a BCA exists on a Type A or a Type C disk On Type B and Type D disks, bit7 shall be set to ZERO.

                for i in range(16):                                                 #/* Bytes 17 to 31
                    control_dat += str(bytearray([int('00000000', 2)]))             #/* These bytes shall be set to (00).

                for i in range(2015):                                               #/* Bytes 32 to 2 047
                    control_dat += str(bytearray([int('00000000', 2)]))             #/* These bytes shall be set to (00).

                for i in range(6):
                    control_dat += str(bytearray([int('00000000', 2)]))
                for i in range(2048):                                               #/* Disk manufacturing information
                    control_dat += str(bytearray([int('00000000', 2)]))             #/* This ECMA Standard does not specify the format and the content of these 2 048 bytes. They shall be ignored in interchange.

                for i in range(14):                                                 #/* Content provider information
                    for j in range(6):                                              #/* The format and the content of these 28 672 bytes require agreement between the interchange parties, else these bytes shall be set to all ZEROs.
                        control_dat += str(bytearray([int('00000000', 2)]))
                    for j in range(2048):
                        control_dat += str(bytearray([int('00000000', 2)]))
            ...
            for menu in self.list_of_menus:
                ...
                if self.dict_of_menus[menu][0].split('.')[-1] == 'png':
                    if self.dict_of_menus[menu][5] and os.path.isfile(self.dict_of_menus[menu][5]):
                        ...
                        sound_file = os.path.join(path_dvdat, 'silence.flac')
...
class main_window(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self.setWindowIcon(QtGui.QIcon(os.path.join(path_graphics, 'dvdat.png')))
...
def open_project_file(self):
    ...
    if not canceled:
        self.actual_project_file = QtGui.QFileDialog.getOpenFileName(self, 'Select the dvdAT project file', path_home, 'Open DVD Producer files (*.dvdat)')[0]#.toUtf8()
        if self.actual_project_file:
            read_project_file(self)

def save_project_file(self):
    if not self.actual_project_file:
        self.actual_project_file = QtGui.QFileDialog.getSaveFileName(self, 'Select a filename to save', path_home, 'Open DVD Producer files (*.dvdat)')[0]
...
def dvd_generate(self):
    self.preview_video_widget.setShown(False)
    if not self.actual_project_file:
        self.generate_dvd_thread_thread.actual_project_file = os.path.join(path_home, self.project_name + '.dvdat')
    else:
...
font_database.addApplicationFont(os.path.join(path_dvdat, 'Ubuntu-RI.ttf'))
font_database.addApplicationFont(os.path.join(path_dvdat, 'Ubuntu-R.ttf'))
font_database.addApplicationFont(os.path.join(path_dvdat, 'Ubuntu-B.ttf'))
font_database.addApplicationFont(os.path.join(path_dvdat, 'Ubuntu-BI.ttf'))
font_database.addApplicationFont(os.path.join(path_dvdat, 'UbuntuMono-R.ttf'))
